...
This program simplifies the checking process by automatically opening images in the right order and recording the user's actions (eg. confirming the match). All the user has to do is to click buttons. After each session, the program will generate a spreadsheet with the correct die grouping, as well as a ``die combination diagram`` for plotting the network of die types in Cytoscape or Gephi. 
